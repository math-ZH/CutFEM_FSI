 ...
 * 	-100: global continous exponential function, u(x,y,z) := (exp(xy), exp(yz), exp(zx))
 *  -1: u(x,y,z) := (cos(xy), cos(yz), cos(zx)) 
 * 			if no_jump == FALSE, then u:= (sin(x+y), sin(y+z), sin(z+x)) when func_no == 1;
	...
	case 0:
      if (sol_order == -100) {
        ...
        res[2] = Cos(z * x);
		    ...
		    res[0] = Pow(x, sol_order) + Pow(y, sol_order);
		    res[1] = Pow(y, sol_order) + Pow(z, sol_order);
		    res[2] = Pow(z, sol_order) + Pow(x, sol_order);
      }
	    break;
	case 1:
...
func_grad_u(FLOAT x, FLOAT y, FLOAT z, FLOAT *res, void *parm)
{
	...
	case 0:
      if (sol_order == -100) {
        ...
        res[0] = sol_order * Pow(x, sol_order - 1.0);
        res[1] = sol_order * Pow(y, sol_order - 1.0);
        res[2] = 0;
        res[3] = 0;
        res[4] = sol_order * Pow(y, sol_order - 1.0);
        res[5] = sol_order * Pow(z, sol_order - 1.0);
        res[6] = sol_order * Pow(x, sol_order - 1.0);
        res[7] = 0;
        res[8] = sol_order * Pow(z, sol_order - 1.0);
	    }
	    break;
	case 1:
	    if (sol_order == -1 && !no_jump) {
  ...
  case 0:
    if (sol_order == -100) {
      ...
      res[2] = -(z * z + x * x) * Cos(z * x) * Coeff(0);    
        ...
        res[0] = res[1] = res[2] = -0. * Coeff(0);
      else if (sol_order > 1) {
        res[0] = -sol_order * (sol_order - 1.) * Coeff(0) *
								(Pow(x, sol_order - 2.) + Pow(y, sol_order - 2.));
				res[1] = -sol_order * (sol_order - 1.) * Coeff(0) *
								(Pow(y, sol_order - 2.) + Pow(z, sol_order - 2.));
        res[2] = -sol_order * (sol_order - 1.) * Coeff(0) *
								(Pow(z, sol_order - 2.) + Pow(x, sol_order - 2.));
			}
    }
    break;
  case 1:
