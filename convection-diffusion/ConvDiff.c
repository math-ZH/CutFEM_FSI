...
// gamma2=1 minimize the cond, but the  
...
static INT added_order = 6;

/* control parameters */
BOOLEAN no_jump = FALSE;

/* order of analytic solution, with some special cases:
 * 	-100: Toroidal flow(polynomial function): u(x,y,z) := ()
 *  -1: Trigonometric function: u(x,y,z) := () 
 * 
 * order of convection velocity:
 * 	1: w(x, y, z) = (1, 0, 0)
 *  2: w(x, y, z) = (-y, x, 0)
 * 
 */
static INT sol_order = -100, conv_type = 1;

static BOOLEAN dump_solver = FALSE, dump_reorder = FALSE;

/* analytic solution */
/* Convection velocity */
static int func_w(FLOAT x, FLOAT y, FLOAT z, FLOAT *res, void *parm)
{
	if (conv_type == 1)
	{
		res[0] = 1;
  ...
  FLOAT X, Y, Z, r;
	...
	case 0:
      if (sol_order == -1) {
        ...
        res[0] = sin(M_PI * x) * cos(M_PI * y) * sin(M_PI * z);
        res[1] = sin(M_PI * y) * cos(M_PI * z) * sin(M_PI * x);
        res[2] = sin(M_PI * z) * cos(M_PI * x) * sin(M_PI * y);
      }
...
func_grad_u(FLOAT x, FLOAT y, FLOAT z, FLOAT *res, void *parm)
{
    ...
    case 0:
      if (sol_order == -100) {
        // Toroidal flow: U = [(c^2 - r_c^2)Y, (c^2 - r_c^2)Z, (c^2 - r_c^2)X]
        X = x - xc; 
        Y = y - yc; 
        Z = z - zc;
        r = X * X + Y * Y + Z * Z;
        ...
        // Trigonometric: U = [sin(πx)cos(πy)sin(πz), 
        //                    sin(πy)cos(πz)sin(πx), 
        //                    sin(πz)cos(πx)sin(πy)]
        FLOAT pi = M_PI;
        FLOAT sx = sin(pi * x), cx = cos(pi * x);
        FLOAT sy = sin(pi * y), cy = cos(pi * y);
        FLOAT sz = sin(pi * z), cz = cos(pi * z);
...
func_f(FLOAT x, FLOAT y, FLOAT z, FLOAT *res, void *parm)
{
  ...
  case 0:
    if (sol_order == -100) {
		// Toroidal flow
			if (conv_type == 1) {
				...
				r = X * X + Y * Y + Z * Z;
				res[0] = -2.0 * X * Y + 10.0 * Y * Coeff(0);
				res[1] = -2.0 * X * Z + 10.0 * Z * Coeff(0);
				res[2] = c * c - r - 2.0 * X * X + 10.0 * X * Coeff(0);
			}
			else if (conv_type == 2) {
				// rotating velocity u = [-y, x, 0]
				X = x - xc; Y = y - yc; Z = z - zc;
				r = X * X + Y * Y + Z * Z;
				res[0] = 2.0 * y * X * Y + x * (c * c - r - 2.0 * Y * Y) + 10.0 * Y * Coeff(0);
				res[1] = 2.0 * y * X * Z - 2.0 * x * Y * Z + 10.0 * Z * Coeff(0);
				res[2] = -y * (c * c - r - 2.0 * X * X) - 2.0 * x * X * Y + 10.0 * X * Coeff(0);
			}
		}
    else if (sol_order == -1) {
    // Trigonometric solution
			if (conv_type == 1) {
			// constant velocity u = [1,0,0]
			res[0] = pi * cos(pi * x) * cos(pi * y) * sin(pi * z) +
								3.0 * pi * pi * sin(pi * x) * cos(pi * y) * sin(pi * z) * Coeff(0);
			res[1] = pi * sin(pi * y) * cos(pi * z) * cos(pi * x) +
								3.0 * pi * pi * sin(pi * y) * cos(pi * z) * sin(pi * x) * Coeff(0);
			res[2] = -pi * sin(pi * z) * sin(pi * x) * sin(pi * y) +
								3.0 * pi * pi * sin(pi * z) * cos(pi * x) * sin(pi * y) * Coeff(0);
			}
   	 	else if (conv_type == 2) {
      // Trigonometric solution with rotating velocity u = [-y, x, 0]
			res[0] = -pi * (y * cos(pi * x) * cos(pi * y) + x * sin(pi * x) * sin(pi * y)) * sin(pi * z) +
								3.0 * pi * pi * sin(pi * x) * cos(pi * y) * sin(pi * z) * Coeff(0);
			res[1] = pi * (-y * sin(pi * y) * cos(pi * z) * cos(pi * x) +
											x * cos(pi * y) * cos(pi * z) * sin(pi * x)) +
								3.0 * pi * pi * sin(pi * y) * cos(pi * z) * sin(pi * x) * Coeff(0);
			res[2] = pi * (y * sin(pi * x) * sin(pi * y) + x * cos(pi * x) * cos(pi * y)) * sin(pi * z) +
								3.0 * pi * pi * sin(pi * z) * cos(pi * x) * sin(pi * y) * Coeff(0);
			}
    }
    else {
...
static int Q_f, Q_gD[3], Q_gN[3], Q_wdotgrad, Q_wBAS;

static int
Op_inprod(int cdim, const FLOAT *coeff, int dim, const FLOAT *in, FLOAT *out,
        QCACHE *qc, int iq, void *ctx)
/* inner product function with function. This is a user operator. */
{
  int out_dim = dim, k;
  ...
  // out_dim = dim / qc->qd->dim;

  // if (in == NULL)
	// 	return out_dim;
	
	// if (qc->qd->dim == 3)
	// 	for (k = 0; k < dim; k += 3, in += 3)
	//     *(out++) = in[0] * coeff[0] + in[1] * coeff[1] + in[2] * coeff[2];
  // else
...
static void
build_linear_system(XFEM_INFO *xi, SOLVER *solver, DOF *u_h[], DOF *f_h[], DOF *w_h[])
{
	GRID *g = u_h[0]->g;
	QCACHE **qc;
	ELEMENT *e;
	...
	for (int k = 0; k < xi->nd; k++) {
		...
		// Q_wdotgrad = phgQCAddFunction_(qc[k], NULL, NULL, func_w, Q_NONE, 1, \
		// 															Op_inprod, Q_GRAD, &k, sizeof(k));
		Q_wBAS = phgQCAddXYZCoefficient(qc[k], func_w, Dim, Q_BAS);
		...
		for (i = 0; i < N; i++) {
			for (j = 0; j < N; j++) {
				...
				val = phgQCIntegrate(qc[rl[k].pno], eno, Q_GRAD, j,
														 qc[rl[k].pno], eno, Q_GRAD, i) * Coeff(rl[k].pno);

				/* \int_T w.\grad u * v */
				val += phgQCIntegrate(qc[rl[k].pno], eno, Q_wBAS, i,
															qc[rl[k].pno], eno, Q_GRAD, j);
				if (LDG){
				/* \int_T \bas u . \bas v */
...
main(int argc, char *argv[])
{
    ...
    phgOptionsRegisterInt("-sol_order", "Analytic solution's polynomial order "
		"(-100: Toroidal flow, -1: Trigonometric function)", &sol_order);
		phgOptionsRegisterInt("-conv_type", "Convection velocity type "
		"(1: constant w=(1,0,0), 2: w=(-y,x,0))", &conv_type);
...
break;
  ...
  //     phgSolverSetPC(solver, pc, NULL);
	...
	t = phgGetTime(NULL);
	f_h = phgXFEMDofNew(xi, DOF_DEFAULT, 3, "f_h", DofNoAction);
	w_h = phgXFEMDofNew0(nd, g, DOF_DEFAULT, 3, "w_h", DofNoAction);
	phgXFEMDofSetDataByFunction(xi, f_h, func_f);
	phgXFEMDofSetDataByFunction(xi, w_h, func_w);
	build_linear_system(xi, solver, u_h, f_h, w_h);
	phgXFEMDofFree(xi, f_h);
	phgXFEMDofFree(xi, w_h);
